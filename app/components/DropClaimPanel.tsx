...
export function DropClaimPanel({ address }: { address: `0x${string}` }) {
  ...
  const badgeLabel = loading
    ? "..."
    : claimableSummary ?? (hasClaimable ? (ru ? "ДОСТУПНО" : "READY") : (ru ? "СКОРО" : "SOON"));
    ...
    <section className={styles.panel} data-state={panelState}>
      ...
      {loading ? (
        ...
        <div className={styles.emptyState}>
          ...
          <p>
            {ru
              ? "После снапшота здесь появятся сумма и кнопка клейма."
              : "Your drop amount and claim button will appear here after the snapshot."}
          </p>
        </div>
      ) : (
          ...
          return (
            <article key={allocation.drop_id} className={styles.drop}>
              ...
              <span className={styles.amount}>
                <small>{ru ? "ТВОЙ ДРОП" : "YOUR DROP"}</small>
                <strong>
